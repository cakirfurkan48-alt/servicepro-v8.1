import {
    ...
    YANIT_PUANLARI,
    PERFORMANS_PUANLARI,
    YetkiliDegerlendirmesi,
    AylikPerformans,
    YetkiliYanit,
    PerformansYanit,
    DEGERLENDIRME_SORULARI,
    GENEL_PERFORMANS_SORULARI,
    PersonelUnvan,
} from '@/types';
...
export function hesaplaBireyselPuan(
    ...
    const zorlukCarpani = IS_TURU_CONFIG[isTuru].carpan;
    let hamPuan: number;

    if (rol === 'sorumlu') {
        // Sorumlu: 40 + (60 × rapor başarısı)
        hamPuan = 40 + (60 * raporBasarisi);
    } else {
        // Destek: Sabit 80 baz puan
        hamPuan = 80;
    }

    let finalPuan = hamPuan * zorlukCarpani;
    ...
    return {
        ...
        zorlukCarpani,
        finalPuan: Math.round(finalPuan),
    };
}
...
export function hesaplaYetkiliPuani(
    sorular: YetkiliDegerlendirmesi['sorular'],
    unvan: PersonelUnvan
): number {
    ...
    // İlk 5 soru
    DEGERLENDIRME_SORULARI.forEach(soruConfig => {
        const yanit = sorular[soruConfig.key as keyof typeof sorular] as YetkiliYanit;
        const puan = YANIT_PUANLARI[yanit];
        const agirlik = isUsta ? soruConfig.ustaAgirlik : soruConfig.cirakAgirlik;
